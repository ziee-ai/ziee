...
if [ -f "$MOUNTED_CONFIG" ]; then
    ...
    envsubst '${ZIEE_DB_HOST} ${ZIEE_DB_PORT} ${ZIEE_DB_USER} ${ZIEE_DB_PASSWORD} ${ZIEE_DB_NAME} ${ZIEE_JWT_SECRET} ${ZIEE_STORAGE_KEY} ${ZIEE_CORS_ALLOW_ORIGIN} ${ZIEE_UPDATE_CHECK} ${ZIEE_LOG_LEVEL} ${ZIEE_LOG_FORMAT} ${ZIEE_CODE_SANDBOX_ENABLED} ${ZIEE_MAX_FILE_UPLOAD_MB} ${ZIEE_PUBLIC_FILE_ORIGIN}' \
        < "$TEMPLATE" > "$RENDERED"
    CONFIG="$RENDERED"
fi
