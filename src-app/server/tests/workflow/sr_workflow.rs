...
/// Full sr-review run, FULLY AUTOMATED (no human gates): seed search → 3
/// auto-expansion rounds (round 1 ADDS, rounds 2-3 EARLY-STOP) → dedup → AI screen
/// → select_included (deterministic) → full-text → extract → synthesize. Every step
/// is mocked; the run completes UNATTENDED (no suspend) and the outputs surface.
#[tokio::test]
async fn sr_review_runs_unattended_to_completion() {
    ...
    let mocks = json!({
        ...
        // select_included derives the AI-included ids from the screen decisions.
        "select_included": {"included_ids": ["10.1/x", "10.3/z"], "included": 2, "excluded": 2, "skipped": 0},
        // Full text for the AI-included set (10.1/x, 10.3/z).
        "fetch": {"papers": [
    ...
    assert_eq!(final_run["status"], "completed", "sr-review runs unattended to completion; got: {final_run}");
    ...
    // The deterministic select_included derived the AI-included set.
    let included = read_step_output(&server, &user.token, run_id, "select_included").await;
    assert_eq!(
        included["included_ids"],
        json!(["10.1/x", "10.3/z"]),
        "the AI-included set is derived deterministically: {included}"
    );
    ...
    let extract = read_step_output(&server, &user.token, run_id, "extract").await;
    assert_eq!(
        extract.as_array().map(|a| a.len()),
        Some(2),
        "AI-extracted evidence table surfaced: {extract}"
    );
}
