...
async fn create_tool_capable_anthropic_model(
    ...
    // Redirect the Anthropic provider at the local LLM bridge via the shared
    // seam (ANTHROPIC_BASE_URL / ZIEE_TEST_LLM_BASE_URL). Without it the provider
    // hits real api.anthropic.com with the placeholder key, the LLM call fails,
    // and the model never emits a tool call ("no mcpToolStart").
