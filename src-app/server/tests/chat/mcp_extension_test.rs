...
async fn test_mcp_tools_added_to_llm_request() {
    ...
    // Create conversation
    let conversation = super::helpers::create_conversation(&server, &admin.token, None, None).await;
    let conversation_id = super::helpers::parse_uuid(&conversation["id"]);
    let branch_id = super::helpers::parse_uuid(&conversation["active_branch_id"]);
...
async fn test_mcp_user_can_only_access_own_servers() {
    ...
    let conversation = super::helpers::create_conversation(&server, &user2.token, None, None).await;
    let conversation_id = super::helpers::parse_uuid(&conversation["id"]);
    let branch_id = super::helpers::parse_uuid(&conversation["active_branch_id"]);
