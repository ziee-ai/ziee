...
async fn test_get_hardware_info_requires_permission() {
    ...
    let admin = crate::common::test_helpers::create_user_with_permissions(&server, "admin", &["hardware::read"]).await;

    // Create regular user without permission
    let user = crate::common::test_helpers::create_user_with_permissions(&server, "regular", &[]).await;
    ...
    assert!(body.get("hardware").is_some(), "Should have hardware object");

    // Verify hardware info structure
    let hardware = body.get("hardware").unwrap();
    assert!(hardware.get("operating_system").is_some(), "Should have OS info");
    assert!(hardware.get("cpu").is_some(), "Should have CPU info");
    assert!(hardware.get("memory").is_some(), "Should have memory info");
    assert!(hardware.get("gpu_devices").is_some(), "Should have GPU devices array");
...
async fn test_get_hardware_info_response_structure() {
    ...
    let os = hardware.get("operating_system").expect("Should have OS info");
    assert!(os.get("name").and_then(|v| v.as_str()).is_some(), "OS should have name");
    assert!(os.get("version").and_then(|v| v.as_str()).is_some(), "OS should have version");
    assert!(os.get("architecture").and_then(|v| v.as_str()).is_some(), "OS should have architecture");

    // Verify CPU info structure
    let cpu = hardware.get("cpu").expect("Should have CPU info");
    assert!(cpu.get("model").and_then(|v| v.as_str()).is_some(), "CPU should have model");
    assert!(cpu.get("cores").and_then(|v| v.as_u64()).is_some(), "CPU should have cores count");
    assert!(cpu.get("architecture").and_then(|v| v.as_str()).is_some(), "CPU should have architecture");

    // Verify memory info structure
    let memory = hardware.get("memory").expect("Should have memory info");
    assert!(memory.get("total_ram").and_then(|v| v.as_u64()).is_some(), "Memory should have total_ram");

    // Verify GPU devices structure (array)
    let gpu_devices = hardware.get("gpu_devices").expect("Should have GPU devices");
...
async fn test_subscribe_hardware_usage_requires_permission() {
    ...
    let admin = crate::common::test_helpers::create_user_with_permissions(&server, "admin", &["hardware::monitor"]).await;
    ...
    assert_eq!(response.status(), 200, "Admin should subscribe to usage stream");
    assert_eq!(
