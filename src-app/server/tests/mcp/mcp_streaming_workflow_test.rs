...
fn count_events_by_type(events: &[crate::chat::helpers::SSEEvent], event_type: &str) -> usize {
    events.iter().filter(|e| e.event == event_type).count()
}

/// Get the indices of events in order
fn get_event_indices(events: &[crate::chat::helpers::SSEEvent], event_types: &[&str]) -> Vec<(String, usize)> {
    events.iter()
        .enumerate()
...
fn assert_single_complete(events: &[crate::chat::helpers::SSEEvent]) {
...
fn assert_event_order(events: &[crate::chat::helpers::SSEEvent], expected_order: &[&str]) {
