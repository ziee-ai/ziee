...
mod tests {
    ...
    fn create_modules_instantiates_all_entries_in_order() {
        ...
        // implementation breaks ties by name on purpose — precisely because
        // linkme's slice order varies between builds. The expectation was
        // therefore linker-dependent and failed for anyone whose edits perturbed
        // link order, with a diff showing the same module names in a different
        // sequence. Matching the implementation's key makes it deterministic and
        // restores what the test is actually for: proving nothing is dropped or
