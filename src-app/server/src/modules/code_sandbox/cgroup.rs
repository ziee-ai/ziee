...
impl CgroupScope {
    pub fn create(parent: &Path, conversation_id: Uuid) -> Result<Self, AppError> {
        ...
        // Defaults matched to the rlimits we set via prlimit inside the
        // sandbox. cgroup is defense-in-depth; rlimits do the heavy
        // lifting when cgroup is unavailable.
        let _ = std::fs::write(path.join("memory.max"), "536870912"); // 512 MiB
        let _ = std::fs::write(path.join("memory.swap.max"), "0");
        let _ = std::fs::write(path.join("pids.max"), "256");
        let _ = std::fs::write(path.join("cpu.max"), "100000 100000"); // 1 CPU
