...
// The DB-free in-memory mount registry + install-cache wipe primitives were
// carved into the sandbox engine; re-import them so the DB-coupled swap-drain
// + delete paths keep operating on the SAME process-global registry.
use ziee_sandbox::registry;
...
pub async fn status(pool: &PgPool) -> Result<VersionStatus, VersionError> {
    ...
    let draining: Vec<DrainEntry> = registry::list_mounted_artifacts()
        .into_iter()
        .map(|m| {
...
pub async fn set_pin_with_drain(
    ...
    let draining: Vec<Arc<registry::MountedArtifact>> = registry::MOUNTED_ARTIFACTS
        .iter()
        .map(|e| e.value().clone())
    ...
    for stale in draining {
        ...
        // spawns the task.
        if !registry::DRAINING_ARTIFACTS.insert(stale.artifact_id) {
            tracing::debug!(
        ...
        tokio::spawn(async move {
            ...
            impl Drop for DrainGuard {
                fn drop(&mut self) {
                    registry::DRAINING_ARTIFACTS.remove(&self.0);
                }
            }
            ...
            stale.wait_until_drained().await;
            tracing::info!(
                artifact_id = %stale.artifact_id,
            ...
            registry::MOUNTED_ARTIFACTS.remove(&stale.artifact_id);

            if policy == SwapPolicy::WipeCachesOnDrain {
                let sentinel = registry::WipeSentinel {
                    old: old_v.clone().unwrap_or_default(),
                    new: new_v.clone(),
                    at: chrono::Utc::now(),
                };
                let result = registry::wipe_install_caches_in_root(&workspace_root, &sentinel);
                tracing::info!(
