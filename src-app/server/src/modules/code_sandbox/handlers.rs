...
// calls to discover available flavors before `execute_command`. The
// legacy REST admin endpoints (`/environments`, `/environments/{f}`,
// `/prefetch`, `/prefetch/{f}/events`) were retired in Plan 5 Phase
// 2c — their function is now served by the version-aware endpoints
// in `version_handlers.rs` + the SSE channel in
// `version_install_tasks.rs`.
// =====================================================================
...
pub async fn update_resource_limits_handler(
    ...
    sync_publish(SyncEntity::CodeSandboxSettings, SyncAction::Update, uuid::Uuid::nil(), None, origin.0);
    Ok((StatusCode::OK, Json(row)))
}
