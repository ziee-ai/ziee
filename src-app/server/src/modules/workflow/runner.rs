...
/// Per-conversation spawn guard for [`spawn_background_run`]
/// (background-spawn-loop-guard). When present AND the run is conversation-bound,
/// the run row is created through the race-safe
/// [`repository::insert_background_run_guarded`] (dedup + concurrency cap); a
/// Duplicate/OverCap short-circuits BEFORE any task is spawned.
#[derive(Debug, Clone, Copy)]
pub struct BackgroundSpawnGuard {
    /// Max non-terminal background runs allowed per conversation (DEC-1 —
    /// `agent_admin_settings.fan_out_max_threads`).
    pub max_active_per_conversation: i64,
    /// Dedup window in seconds (DEC-2 — `SPAWN_DEDUP_WINDOW_SECS`).
    pub dedup_window_secs: i64,
}

/// The outcome of a (possibly guarded) [`spawn_background_run`].
#[derive(Debug)]
pub enum BackgroundSpawnResult {
    /// A run was created and its detached task started.
    Spawned(Uuid),
    /// The guard found an identical in-flight/recent run — NOTHING was spawned;
    /// carries the existing run's id (INV-1).
    Duplicate(Uuid),
    /// The conversation is at its concurrency cap — NOTHING was spawned (INV-2).
...
pub async fn spawn_background_run<F, Fut>(
    ...
    guard: Option<BackgroundSpawnGuard>,
    driver: F,
) -> Result<BackgroundSpawnResult, AppError>
where
    ...
    Fut: std::future::Future<Output = BackgroundOutcome> + Send + 'static,
{
    ...
    // Create the durable row (workflow_id = NULL). When a guard is supplied for a
    // conversation-bound run, the DEDUP + CAP checks and the INSERT happen
    // atomically in one txn (background-spawn-loop-guard); a Duplicate/OverCap
    // returns here WITHOUT registering a handle or spawning a task. Otherwise the
    // plain insert preserves the original detached/scheduler behavior.
    let row = match (guard, request.conversation_id) {
        (Some(g), Some(_)) => {
            match repository::insert_background_run_guarded(
                pool,
                request,
                g.max_active_per_conversation,
                g.dedup_window_secs,
            )
            .await?
            {
                repository::GuardedBackgroundInsert::Inserted(row) => *row,
                repository::GuardedBackgroundInsert::Duplicate(id) => {
                    return Ok(BackgroundSpawnResult::Duplicate(id));
                }
                repository::GuardedBackgroundInsert::OverCap { active } => {
                    return Ok(BackgroundSpawnResult::OverCap {
                        active,
                        cap: g.max_active_per_conversation,
                    });
                }
            }
        }
        _ => repository::insert_background_run(pool, request).await?,
    };

    // Register the handle and flip `pending → running` through the SAME guarded
    // transitions the workflow runner uses (a fast owner-cancel that landed first
    // makes `mark_running` a no-op — the driver observes cancel via the handle).
    let run_id = row.id;
    ...
    Ok(BackgroundSpawnResult::Spawned(run_id))
}
