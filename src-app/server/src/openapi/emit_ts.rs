...
mod tests {
    ...
    /// Same guard for the desktop UI, whose `types.ts` is generated by the
    /// `ziee-desktop` binary from the combined (server + desktop) spec — but
    /// through this exact generator, so it must stay byte-identical too.
    #[test]
