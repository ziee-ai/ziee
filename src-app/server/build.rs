...
fn setup_external_binaries() {
    ...
    if let Err(e) = pgvector_build::build_pgvector(&target, &out_dir_path) {
        ...
        // stderr unless the script itself fails or you pass `-vv`, so the reason
        // this failed was invisible in CI. It matters more than the old wording
        // admitted: without pgvector the app does not merely lose memory
        // features, it FAILS TO BOOT, because the first framework migration
        // (202607140001_app_bootstrap.sql) does an unconditional
        // `CREATE EXTENSION vector`.
        //
        // One line per warning — cargo truncates each at the first newline.
