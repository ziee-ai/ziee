...
fn main() {
    ...
    // Generate the extensions file (for field composition)
    let out_dir = env::var("OUT_DIR").unwrap();
    let dest_path = PathBuf::from(&out_dir).join("chat_extensions.rs");
    generate_extensions_file(&extensions, &dest_path);

    // Also generate a module file in src for auto-registration
    let auto_gen_path = server_dir.join("src/modules/chat/extension_registration.rs");
    generate_auto_registration_module(&extensions, &auto_gen_path);
...
fn generate_extensions_file(extensions: &[ExtensionInfo], dest_path: &PathBuf) {
    // Generate field definitions for the proc macro
...
// This is a token stream representation of fields to be added
...
fn generate_auto_registration_module(extensions: &[ExtensionInfo], dest_path: &PathBuf) {
    // Generate registration code
    let registration_code = extensions
        .iter()
        .map(|ext| {
            // Strip "extensions::" prefix from module_path to avoid duplication
            let module_path = ext.module_path
                .strip_prefix("extensions::")
                .unwrap_or(&ext.module_path);

            format!(
                "    // {} (order: {})\n    registry.register(crate::modules::chat::extensions::{}::extension::create(pool.clone()));",
                ext.name, ext.order, module_path
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
