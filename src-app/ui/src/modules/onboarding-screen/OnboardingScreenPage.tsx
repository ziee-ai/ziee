...
export default function OnboardingScreenPage() {
  ...
  const nextEnabled = Stores.OnboardingScreen.nextEnabled
  const nextLoading = Stores.OnboardingScreen.nextLoading
  const nextError = Stores.OnboardingScreen.nextError
  ...
  useEffect(() => {
    Stores.OnboardingScreen.setReady(currentStep?.skippable !== false)
    Stores.OnboardingScreen.setNextError(null)
    beforeNextRef.current = null
  }, [currentStepIndex, activeGuideId])

  const handleGlobalNext = useCallback(async () => {
    if (!guide) return
    Stores.OnboardingScreen.setNextLoading(true)
    Stores.OnboardingScreen.setNextError(null)
    try {
      await beforeNextRef.current?.()
      await Stores.OnboardingScreen.completeStep(guide.id, guide.steps[currentStepIndex].id)
      if (currentStepIndex === guide.steps.length - 1) {
        await Stores.OnboardingScreen.completeGuide(guide.id)
        Stores.OnboardingScreen.reset()
        Stores.ApiKeysStep.reset()
        Stores.McpServersStep.reset()
      ...
      Stores.OnboardingScreen.setNextError(err.message || 'Something went wrong')
    } finally {
      Stores.OnboardingScreen.setNextLoading(false)
    }
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {nextError && (
            <Alert
              ...
              showIcon
              closable
              onClose={() => Stores.OnboardingScreen.setNextError(null)}
              className="mb-4"
            />
          )}
          {StepComponent && (
