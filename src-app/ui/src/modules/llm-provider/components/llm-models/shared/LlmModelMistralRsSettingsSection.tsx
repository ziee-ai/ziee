...
const fieldName = (field: string) => `engine_settings.mistralrs.${field}`
...
export function LlmModelMistralRsSettingsSection() {
  ...
  const selectedDeviceType =
    useWatch({ name: fieldName('device_type') }) || 'cpu'
  const currentQuantization = useWatch({ name: fieldName('in_situ_quant') })
  ...
  useEffect(() => {
    if (currentQuantization) {
      ...
      if (!isCurrentQuantizationCompatible) {
        form.setValue(fieldName('in_situ_quant'), undefined)
      }
    }
    ...
    <Flex vertical className="gap-4 w-full">
      ...
      <Card title="Sequence & Memory Management">
        <Flex vertical className="gap-2 w-full">
          ...
          >
            <FormField name={fieldName('max_seqs')} className="m-0">
              <InputNumber min={1} max={1024} placeholder="16" />
            </FormField>
          </ResponsiveConfigItem>
      ...
      <Card title="PagedAttention Configuration">
        <Flex vertical className="gap-2 w-full">
          ...
          >
            <FormField name={fieldName('paged_attn_gpu_mem')} className="m-0">
              <InputNumber min={128} max={65536} placeholder="Auto" />
            </FormField>
          </ResponsiveConfigItem>
          ...
          >
            <FormField name={fieldName('paged_attn_gpu_mem_usage')} className="m-0">
              <InputNumber min={0.1} max={1.0} step={0.1} placeholder="0.9" />
            </FormField>
          </ResponsiveConfigItem>
          ...
          >
            <FormField name={fieldName('paged_ctxt_len')} className="m-0">
              <InputNumber min={512} max={131072} placeholder="Auto" />
            </FormField>
          </ResponsiveConfigItem>
          ...
          >
            <FormField name={fieldName('paged_attn_block_size')} className="m-0">
              <InputNumber min={1} max={512} placeholder="32" />
            </FormField>
          </ResponsiveConfigItem>
      ...
      <Card title="Performance Optimization">
        <Flex vertical className="gap-2 w-full">
          ...
          >
            <FormField name={fieldName('prefix_cache_n')} className="m-0">
              <InputNumber min={1} max={128} placeholder="16" />
            </FormField>
          </ResponsiveConfigItem>
        </Flex>
      </Card>
      ...
      <Card title="Model Configuration">
        <Flex vertical className="gap-2 w-full">
          ...
          >
            <FormField name={fieldName('dtype')} className="m-0">
              <Select
                placeholder="auto"
                allowClear
                clearLabel="Clear"
          ...
          >
            <FormField name={fieldName('in_situ_quant')} className="m-0">
              <Select
                placeholder="None"
                allowClear
                clearLabel="Clear"
          ...
          >
            <FormField name={fieldName('seed')} className="m-0">
              <InputNumber min={0} max={4294967295} placeholder="Random" />
            </FormField>
          </ResponsiveConfigItem>
        </Flex>
      </Card>
