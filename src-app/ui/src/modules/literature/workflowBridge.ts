...
/**
 * Bridge a completed SR `sr-search-screen` / `sr-snowball-screen` workflow run
 * into the literature screening panel — reading the run's `candidates`
 * (deduped records + PRISMA counts) and `ai_screening` (per-record first-pass
 * decisions) outputs, seeding include/exclude from the AI pass so the human
 * refines from there. Returns false when the run has no screening-shaped output
 * (so the caller can surface a hint instead of opening an empty panel).
 */
...
export async function openWorkflowScreening(runId: string): Promise<boolean> {
  ...
  // complete step outputs live on disk, served by `readOutput`. The candidate
  // set is produced by the `search` step (sr-search-screen) or `snowball` step
  // (sr-snowball-screen); the AI first-pass decisions by the `screen` step.
  const stepOutputs = (run?.step_outputs_json ?? {}) as Record<string, unknown>
  const candStep = ['search', 'snowball'].find(s => s in stepOutputs)
