...
export function BackgroundTasksPage() {
  ...
  const { runs, total, currentPage, pageSize, loading, error } =
    Stores.BackgroundRuns

  useEffect(() => {
    void Stores.BackgroundRuns.loadRuns(1)
  }, [])
  ...
  useEffect(() => {
    if (error && runs.length > 0) {
      message.error(error)
      Stores.BackgroundRuns.clearError()
    }
  }, [error, runs.length])
            ...
            <div className="flex w-full max-w-4xl flex-col gap-3 self-center px-3 pt-3">
              {loading && runs.length === 0 ? (
                ...
                <ErrorState
                  ...
                  details={error}
                  onRetry={() => void Stores.BackgroundRuns.loadRuns(currentPage)}
                ...
                <>
                  ...
                  <ListPagination
                    ...
                    pageSize={pageSize}
                    itemNoun="tasks"
                    onChange={page => Stores.BackgroundRuns.setPage(page, pageSize)}
                    onPageSizeChange={size => Stores.BackgroundRuns.setPage(1, size)}
