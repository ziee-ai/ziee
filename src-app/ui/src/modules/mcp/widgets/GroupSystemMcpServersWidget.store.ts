...
export const useGroupSystemMcpServersWidgetStore =
  create<GroupSystemMcpServersWidgetState>()(
        ...
        (set, get): GroupSystemMcpServersWidgetState => ({
            ...
            __store__: () => {
              ...
              eventBus.on(
                ...
                async event => {
                  ...
                  const assignedServers = allServers.filter(s =>
              ...
              eventBus.on(
                ...
                async event => {
                  ...
                  if (server.is_system) {
                    set(state => {
                      const index = state.allServers.findIndex(
                        s => s.id === server.id,
                      )
                      if (index !== -1) {
              ...
              eventBus.on(
                ...
                async event => {
                  ...
                  set(state => {
                    ...
                    state.allServers = state.allServers.filter(
                      s => s.id !== serverId,
                    )

                    // Clear it from all groupServers maps
                    state.groupServers.forEach((groupData, groupId) => {
                      const updatedServers = groupData.servers.filter(
                        s => s.id !== serverId,
                      )
                      state.groupServers.set(groupId, {
                        ...groupData,
          ...
          loadServersForGroup: async (
            ...
            try {
              ...
              for (const server of allServers) {
                const groupIds =
                  await ApiClient.McpServerSystem.getServerGroups({
                    id: server.id,
                  })
              ...
              console.error(
                `Failed to load servers for group ${groupId}:`,
                error,
              )
          ...
          __destroy__: () => {
            Stores.EventBus.removeGroupListeners(
              'GroupSystemMcpServersWidgetStore',
            )
          },
        }),
      ),
