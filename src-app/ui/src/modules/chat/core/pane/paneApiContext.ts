 ...
 *
 * `ChatPaneProvider` provides its per-pane store's raw api here; the `Stores.Chat`
 * bridge reads it during a reactive render so that EVERY existing
 * `Stores.Chat.<field>` reactive read inside a pane subtree resolves to THAT
 ...
 * byte-identical.
 *
 * (Handler-time reads/actions — `Stores.Chat.$.x` / `Stores.Chat.doThing()` — run
