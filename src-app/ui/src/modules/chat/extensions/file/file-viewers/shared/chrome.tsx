...
export function RawToggle({ file }: { file: FileEntity }) {
  ...
  const mode = Stores.Chat.FileStore.fileViewModes.get(file.id) ?? 'compiled'
  return (
    <Space.Compact>
      <Button
        icon={<EyeOutlined />}
        ...
        onClick={() => Stores.Chat.FileStore.setFileViewMode(file.id, 'compiled')}
      />
      <Button
        icon={<CodeOutlined />}
        ...
        title="Raw view"
        aria-label="Raw view"
        onClick={() => Stores.Chat.FileStore.setFileViewMode(file.id, 'raw')}
      />
    </Space.Compact>
  )
}
...
export function DownloadButton({ file }: { file: FileEntity }) {
  ...
  return (
    <Button
      ...
      onClick={() => {
        Stores.Chat.FileStore.downloadFile(file).catch(() =>
