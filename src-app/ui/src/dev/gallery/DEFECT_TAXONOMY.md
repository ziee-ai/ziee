...
- A12 [G] **cramped nested borders (redundant double border)** — a bordered CONTROL (button/input/select/link) whose border-box edge sits within ~10px of its nearest bordered ANCESTOR's inner (padding-box) edge on one/both sides: the container's border and the control's outline paint two near-parallel strokes that read as a crowded double border. Report the side + measured gap. The usual fix is a quiet/**ghost** variant instead of an outline inside an already-bordered container. Scoped to controls (that's where the outline-in-a-box anti-pattern lives), not every full-bleed child of a card. *(edit-conversation-title + edit-provider-name outline buttons crammed to their header/container edges.)*
...
- J9 [L/G] **inconsistent tooltip placement among peer buttons** — within ONE button-group / toolbar / header-actions row every button's tooltip must open toward the SAME side; a row where some drop DOWN and others pop UP reads as sloppy and the downward ones can cover the body below. [L] lint (`scripts/lint-tooltip-placement.mjs`, AST — no render needed since tooltips are hover-gated): resolve each button's tooltip side from `<Tooltip side=…>` (default `top`) or `<Button tooltip=… tooltipSide=…>` (default `top`) and flag a container holding ≥2 with non-uniform sides. [G] alt: a hover recipe hovers each peer and compares the tooltip's side vs its trigger. *(the file-viewer header: Rendered/Raw view toggles use `<Tooltip>` = top while Copy/Download pass `tooltipSide="bottom"`.)*
...
7. **BOUNDARY-VALUE SEEDING** (the shared coverage backbone): every data-driven surface (tables/lists/viewers/message states/menus/pickers) must be seeded at the boundaries `{0, 1, few, many}` items × viewports — NOT one middle case. Most recent misses only appear at a boundary: dead space (A14) at a FEW-row table, empty-picker (H7) at ZERO models, code-sandbox label wrap (C9) at a NARROW width, alignment break (A13) at a message WITH attachments. A surface rendered only at its comfortable middle size hides its degenerate ends. Known-positive boundary fixtures for the geometrically/runtime-expressible classes live in `DefectRepro.tsx` (the detector-acceptance harness); real data-surfaces should carry `{empty, one, few, many}` seed variants where feasible.
