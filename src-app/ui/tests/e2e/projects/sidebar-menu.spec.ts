...
test.describe('Sidebar conversation menu — project contributions', () => {
  ...
  test('"Add to project" menu item opens AddToProjectModal and attaches on confirm', async ({
    ...
    // Type to filter: the unfiltered Base-UI combobox list virtualizes/animates,
    // leaving options "not stable" to click. Filtering to the one match lets the
    // option settle so a normal click registers the selection.
    await byTestId(dialog, 'project-add-to-project-combobox').fill('Sidebar Add Target')
    await byTestId(
      page,
