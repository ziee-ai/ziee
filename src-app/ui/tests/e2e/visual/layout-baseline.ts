...
export const LAYOUT_BASELINE: LayoutBaselineEntry[] = [
  ...
  },
  {
    ...
    note: "Pre-existing on origin/main (proven apples-to-apples: reproduces IDENTICALLY — same 20.8px, ×4 — on a clean origin/main gallery, and this branch touches ZERO mermaid/markdown files). The MermaidBlock rendered SVG is a fixed-intrinsic-width diagram (~410px) that does not reflow, so it overflows its parent by ~20.8px at the 390px mobile viewport. The render container carries no data-testid, so this is section+check-keyed. Fix path: wrap MermaidBlock's render container in max-w-full + overflow-x-auto — a markdown-rendering decision, out of scope for split-chat-multipane.",
  },
