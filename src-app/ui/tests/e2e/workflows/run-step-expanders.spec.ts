...
test.describe('Workflows - run progress per-step log expanders', () => {
  ...
  // TEST-11 — "Show full output" now routes its markdown through the shared
  // `preprocessMarkdown`. Uses the SAME dev-only `mock:` short-circuit as the
  // test above, so the markdown is the step's REAL persisted output travelling
  // the real read-output path — nothing is mocked at the HTTP layer.
  test('mocked step: Show full output renders LaTeX math and leaves code literal', async ({
    page,
    request,
    testInfra,
  }) => {
    ...
    // The mock value IS the step output. It carries a display equation, an
    // inline one, and a fenced block whose `\[` must stay literal.
    const mathYaml = `$schema: "/schemas/2026-06-12/workflow-definition.schema.json"
inputs: []
steps:
  - id: derive
    kind: llm
    description: "Derive the steady-state profile"
    prompt: |
      Say one short sentence.
    mock: |
      Steady state:
...
`
    ...
    await goToWorkflowsSettingsPage(page, baseURL)
    await openWorkflowCard(page, 'e2e-expander-math')
    ...
    // Expand "Show full output" → lazily fetches the persisted output bytes.
    const outputAccordion = page
      .locator('[data-testid^="wf-step-output-accordion-"]')
      .first()
    await expect(outputAccordion).toBeVisible({ timeout: 15000 })
    await outputAccordion.getByText('Show full output').click()

    // The display equation renders as a KaTeX block, the inline one as inline
    // KaTeX (2 total) — and the raw LaTeX is gone from the prose.
    await expect(outputAccordion.locator('.katex-display').first()).toBeVisible({
      timeout: 15000,
    })
    expect(
      await outputAccordion.evaluate(el => el.querySelectorAll('.katex').length),
    ).toBe(2)
    await expect(outputAccordion).not.toContainText('\\frac{d^2C(x)}{dx^2}')

    // ...while the fenced block keeps its delimiters verbatim.
    await expect(
      outputAccordion.locator('[data-streamdown="code-block"]'),
