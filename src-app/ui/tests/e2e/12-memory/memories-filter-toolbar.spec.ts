 ...
 * (`GET /api/memories?kind=<kind>`, Memories.store.ts:79-81,260-265). No prior
 * spec exercised it. This seeds two memories of different kinds, picks a kind in
 * the toolbar, and asserts the list filters to that kind via the real fetch.
 */
...
test.describe('Memory — My memories filter toolbar', () => {
  test('selecting a Kind in the toolbar filters the list server-side', async ({
    ...
    await expect(page.getByText(factText)).toBeVisible({ timeout: 10_000 })
    ...
    await page
      .locator(
        '.ant-select:has(.ant-select-selection-placeholder:text-is("Kind"))',
      )
      .click()
    await page.getByTitle('Goal', { exact: true }).click()
    await kindReq

    // Only the goal memory remains; the fact memory is filtered out.
    await expect(page.getByText(goalText)).toBeVisible({ timeout: 10_000 })
