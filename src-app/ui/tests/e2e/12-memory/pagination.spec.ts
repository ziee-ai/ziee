 ...
 *
 * The section paginates at pageSize=10 (Pagination with a "X-Y of N
 * memories" total + page-size options). With >10 memories seeded, the first
 * page shows 10, and navigating to page 2 reveals the remaining rows (a
 * memory that was NOT on page 1). Existing memory specs cover
...
test.describe('Memory — pagination', () => {
  test('paginates the memory list past the first page', async ({ page, testInfra }) => {
    ...
    // Page 1 shows 10 rows; the Pagination has a "2" page button.
    const rows = page.locator('[data-memory-id]')
    await expect(rows).toHaveCount(10, { timeout: 15000 })
    const page2 = byTestId(page, 'memory-pagination-page-2')
    await expect(page2).toBeVisible()

    // Go to page 2 → the remaining 2 rows.
    await page2.click()
    ...
    await expect(rows.filter({ hasText: /PAGEMEM_0[01] / })).toHaveCount(2)
  })
  ...
  test('the page-size changer reflows all rows onto one page', async ({
    ...
    await expect(page.locator('[data-memory-id]')).toHaveCount(10, { timeout: 15000 })

    // Bump the page size to "20 / page" via the size changer → all 12 fit.
