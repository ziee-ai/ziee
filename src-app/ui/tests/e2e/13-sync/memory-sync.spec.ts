...
async function addMemoryViaUI(page: Page, content: string) {
  ...
  await expect(page.getByText('Memory added')).toBeVisible({ timeout: 5_000 })
}
...
test.describe('Realtime sync — memory_settings (owner-scoped)', () => {
  test('a memory preference toggled on device A is reflected on the same user device B', async ({
    ...
    try {
      ...
      const switchA = page.getByRole('switch').first()
      const switchB = pageB.getByRole('switch').first()
      ...
      await page
        .locator('.ant-card', { has: page.getByRole('switch') })
...
test.describe('Realtime sync — memory_admin_settings (permission-scoped)', () => {
  test('toggling deployment memory on device A reflects on admin device B but not a non-admin', async ({
    ...
    try {
      ...
      const enableA = page.getByRole('switch').first()
      const enableAdminB = pageAdminB.getByRole('switch').first()
      ...
      await page
        ...
        .getByRole('button', { name: /^Save$/ })
        .click()
      await expect(page.getByText('Engine saved.')).toBeVisible({
        timeout: 5_000,
      })
