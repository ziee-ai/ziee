...
export async function openGroupAssignmentDrawerFromProvider(page: Page) {
  // Should already be on provider detail page. The card wraps the shared
  // UserGroupAssignment (testid prefix `llm-provider-groups`): the "Assign"
  // action opens the editor Drawer.
  await byTestId(page, 'llm-provider-groups-card').waitFor({ state: 'visible', timeout: 10000 })
  await byTestId(page, 'llm-provider-groups-assign').click()
  await byTestId(page, 'llm-provider-groups-save').waitFor({ state: 'visible', timeout: 5000 })
}
...
export async function toggleGroupInDrawer(page: Page, groupName: string, enable: boolean) {
  ...
  const switchElement = groupContainer.locator('[data-testid^="llm-provider-groups-drawer-switch-"]').first()
  const isChecked = (await switchElement.getAttribute('aria-checked')) === 'true'
  if (isChecked !== enable) {
    await switchElement.click()
    await page.waitForTimeout(300)
...
export async function saveGroupAssignment(page: Page) {
  const saveBtn = byTestId(page, 'llm-provider-groups-save')
  await saveBtn.click()
...
export async function cancelGroupAssignment(page: Page) {
  await byTestId(page, 'llm-provider-groups-cancel').click()
  await byTestId(page, 'llm-provider-groups-save').waitFor({ state: 'hidden', timeout: 5000 })
}
...
// The shared UserGroupAssignment renders the assigned-group tags only when its
// disclosure body is expanded; expand it before asserting on the tags.
async function expandProviderGroupsBody(page: Page) {
  const card = byTestId(page, 'llm-provider-groups-card')
  await card.waitFor({ state: 'visible', timeout: 5000 })
  const toggle = byTestId(page, 'llm-provider-groups-toggle')
...
export async function assertGroupInProviderCard(page: Page, groupName: string) {
  ...
  const card = await expandProviderGroupsBody(page)

  const groupTag = card
    .locator('[data-testid^="llm-provider-groups-tag-"]')
    .filter({ hasText: groupName })
    .first()
...
export async function assertProviderCardShowsCount(page: Page, expectedCount: number) {
  ...
  const tags = card.locator('[data-testid^="llm-provider-groups-tag-"]')
  await expect(tags).toHaveCount(expectedCount)
}
