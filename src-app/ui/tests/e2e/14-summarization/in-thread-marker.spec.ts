...
test.describe('Summarization — in-thread boundary marker', () => {
  ...
  test('renders divider at the anchor message and expands to show summary', async ({
    ...
    // `message_footer` slot. Look for the marker's distinctive copy.
    const divider = page
      .locator(`[data-message-id="${anchorMessageId}"]`)
      .getByText(/Earlier 3 messages condensed into a summary/i)
    await expect(divider).toBeVisible({ timeout: 30000 })

    // Click the divider to expand → the summary text card appears.
    await divider.click()
    await expect(
      page.getByText(
        'The user wants to plan a Tokyo trip and is asking about neighborhoods.',
    ...
    // And the model + timestamp line is rendered.
    await expect(page.getByText(/Generated by test-model at/)).toBeVisible({
      timeout: 5000,
    })
  })
})
