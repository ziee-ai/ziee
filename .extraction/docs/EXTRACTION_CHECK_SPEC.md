...
- **EA merged-migrator** (chunk MIGRATE-squash, deterministic; supersedes the pre-N3.1 BA form): the merged Migrator — composed from `modules/*/migrations/ ∪ sdk/crates/*/migrations/` (module-owned, `<YYYYMMDDNNNN>_<module>_<desc>.sql`, monotonic version) — applies on a FRESH DB with no checksum errors, and satisfies TWO LOGICAL anchors (NOT byte-identical, per B1 — a squash reorders CREATE/ALTER so `pg_dump` byte-differs on a logically-identical schema):
  - **EA-schema:** a **catalog-derived structural fingerprint** (from `information_schema`/`pg_catalog`) is IDENTICAL to `.extraction/baseline/schema.fp`: tables as a set of `{column → (type, nullable, normalized-default, **is_generated + generation_expression**)}` (attnum-order-independent — the generated-column expr is REQUIRED: ziee has 3 `GENERATED` columns, `content_tsv`×2 + a `user_memories` col, that a bare type-tuple would miss); **indexes via name-normalized `pg_get_indexdef`** (captures opclass e.g. `halfvec_cosine_ops` + partial predicate + method); **constraints via `pg_get_constraintdef`** (FK target+columns+`ON UPDATE`/`ON DELETE`/`DEFERRABLE`, CHECK expr, PK/UNIQUE col-sets) with auto-name suffixes normalized off; enums (name+ordered values), sequences, functions, triggers, extensions by definition. (RLS/GRANT/COMMENT/collation deliberately excluded — not used in a way that affects this codebase.) Validator RE-RUNS the fingerprint script on both the baseline and the squashed DB and diffs (unfakeable).
  - **EA-seed:** a **whole-DB data image** (all rows, all tables — a fresh-migrated DB holds only seed rows) is equivalent to `.extraction/baseline/seed.sql`. Compare **algorithm** (the baseline is NOT literal-id-stable — some seed rows use `gen_random_uuid()`): per table, (1) drop volatile columns (`created_at`/`updated_at` and any `gen_random_uuid()` surrogate PK) from the compared tuple; (2) key each row by its **business key** (natural unique cols — e.g. `groups.name`, `auth_providers.name`, a settings singleton's fixed discriminator), NOT the surrogate uuid; (3) resolve FK columns **through the referenced row's business key** (join, don't compare raw uuids), so a differently-generated-but-equivalent uuid graph matches; (4) **element-sort set-valued array columns** before comparing — `groups.permissions` is `TEXT[]` and order is not significant, so a raw compare would false-fail. Missing/extra/business-key-mismatched/content-differing row → fail. (Rows with neither a business key nor stable id are flagged for explicit author review, not silently passed.)
