#!/bin/bash
# Golden check: regen openapi/types for both surfaces, compare to baseline.
# types.ts -> byte-identical ; openapi.json -> canonical set-equality (jq -S)
set -e
export CARGO_TARGET_DIR=/data/pbya/ziee/tmp/sdk-extraction-target
WT=/data/pbya/ziee/tmp/sdk-extraction-wt
cd $WT/src-app
DB="postgresql://postgres:password@127.0.0.1:54321/ziee_build_3533975a"
echo ">>> regen UI spec"
DATABASE_URL="$DB" CONFIG_FILE=server/config/openapi-gen.yaml cargo run --bin ziee -- --generate-openapi ui/openapi >/tmp/regen-ui.log 2>&1 || { echo "UI REGEN BUILD FAILED"; tail -30 /tmp/regen-ui.log; exit 3; }
echo ">>> regen desktop spec"
DATABASE_URL="$DB" CONFIG_FILE=server/config/openapi-gen.yaml cargo run -p ziee-desktop -- --generate-openapi desktop/ui/openapi >/tmp/regen-desktop.log 2>&1 || { echo "DESKTOP REGEN BUILD FAILED"; tail -30 /tmp/regen-desktop.log; exit 3; }
