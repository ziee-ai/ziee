...
- **TEST-22** (tier: e2e) [covers: ITEM-11, ITEM-1] file: `src-app/ui/tests/e2e/14-split-chat/open-in-split-from-list.spec.ts` — asserts: the recent-conversations "Open in split" affordance opens that conversation as a second pane; opening beyond `MAX_PANES` is prevented.
...
- **TEST-37** (tier: e2e) [covers: ITEM-11, ITEM-1] file: `src-app/ui/tests/e2e/14-split-chat/new-chat-no-hijack.spec.ts` — asserts: creating a conversation in a second (new-chat) pane adopts it into THAT pane and does NOT navigate the whole window away (no window-hijack; the other pane stays put).
- **TEST-38** (tier: e2e) [covers: ITEM-6] file: `src-app/ui/tests/e2e/14-split-chat/pane-switch-conversation.spec.ts` — asserts: switching a pane's conversation loads a FRESH stream/message state for the new conversation without leaking the old one's buffered frames.
...
- **TEST-43** (tier: e2e) [covers: ITEM-7] file: `src-app/ui/tests/e2e/14-split-chat/find-per-pane.spec.ts` — asserts: the find toggle opens the find bar in the FOCUSED pane only (not both) — the find affordance is pane-scoped.
