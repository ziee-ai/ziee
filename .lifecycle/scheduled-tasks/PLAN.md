...
- **ITEM-2**: Replace the raw IANA-string timezone `<Input>` in `ScheduleBuilder` with a searchable timezone Combobox (IANA zones from `Intl.supportedValuesOf('timeZone')`, default = browser tz). (FB-2, same "never make users type an opaque value" spirit)
...
- **ITEM-4**: For a workflow target, render the selected workflow's typed inputs via the existing `parseWorkflowIr` (reuse `WorkflowRunDialog`'s typed-input rendering) with a JSON fallback when the IR exposes no inputs, replacing the raw "Inputs (JSON)" textarea. (FB-1 + mitigates backend Bug 10)
