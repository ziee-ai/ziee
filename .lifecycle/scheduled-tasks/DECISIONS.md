...
### DEC-19: Storage + API shape for the per-task allow-list + skipped-tools report (migration 146)?
**Resolution:** Add migration `146` (next after main's `145`): `scheduled_tasks.allowed_unattended_tools JSONB NOT NULL DEFAULT '[]'` (list of `{server_id, tool_name?}` — tool_name NULL ⇒ whole server allowed), and `scheduled_task_runs.skipped_tools JSONB NOT NULL DEFAULT '[]'` (list of `{tool_name, reason}`). `CreateScheduledTask`/`UpdateScheduledTask` gain `allowed_unattended_tools`; `ScheduledTaskRun` exposes `skipped_tools`. This is a new type surface → OpenAPI regen (BOTH ui + desktop) is required.
