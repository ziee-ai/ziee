...
## Round 3 (FB-9 precedent audit) — Decisions

### DEC-24: Task-list layout — single-column expandable rows or 2-column card grid?
**Resolution:** single-column expandable rows (keep today's model; runs expand in place)
**Basis:** user — explicit picker choice (2026-07-12); task rows are info-dense and expand to a run timeline, so a 2-col grid was rejected.

### DEC-25: Card action buttons — always-visible or hover-revealed?
**Resolution:** hover/focus-revealed `outline size="icon"` + `Tooltip` (mirror `ProjectCard`); the enable/disable `Switch` stays always-visible as it conveys state, not an action.
**Basis:** user — explicit picker choice (2026-07-12).

### DEC-26: Form drawer field/validation idiom — tidy to FormField + zodResolver, minimal, or leave as-is?
**Resolution:** full tidy-up — `zodResolver(schema)` + wrap the stray Segmented/Schedule/notify controls in `FormField`/`Field`, matching `ProjectFormDrawer`.
**Basis:** user — explicit picker choice (2026-07-12).

### DEC-27: Per-task detail page — add `/scheduled-tasks/:id` this round, or defer?
**Resolution:** defer — a detail page is its own follow-up lifecycle; NOT in iteration-2 scope.
**Basis:** user — verbatim: "we add a detail page then we will need a follow up different life cycle to develop this page" (2026-07-12).

## Round 3 — Descope dispositions
- DESCOPED: ITEM-60 — 2-column card grid; human chose single-column expandable rows (DEC-24) [approved: human 2026-07-12]
- DESCOPED: ITEM-61 — per-task detail page; human directed it to a separate future lifecycle (DEC-27) [approved: human 2026-07-12]
