...
- **ITEM-2 (timezone picker in ScheduleBuilder):** `ScheduleBuilder` is used only by the drawer; `value.timezone` stays a string. No other consumer.
...
- **ITEM-2** — timezone Combobox from `Intl.supportedValuesOf('timeZone')`; kit `Combobox` already used elsewhere. No existing tz-picker to mirror, but the Combobox usage matches `AssistantSelector`.
...
- **ITEM-2** — verdict: PASS — timezone Combobox mirrors existing Combobox usage; value stays a string.
