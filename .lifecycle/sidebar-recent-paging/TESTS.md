...
- **TEST-11** (tier: e2e) [covers: ITEM-6, ITEM-9] file: `src-app/ui/tests/e2e/chat/sidebar-recent-infinite-scroll.spec.ts` — asserts: VIRTUALIZATION — with 45 loaded, the number of row elements in the DOM is bounded to a window (materially fewer than the loaded count, e.g. ≤ a small overscan-bounded number), and the list container exposes `role="list"` with rows carrying `aria-setsize`/`aria-posinset`.
