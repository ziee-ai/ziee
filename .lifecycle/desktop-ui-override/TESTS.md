...
- **TEST-8** (tier: unit) [covers: ITEM-8] file: `src-app/desktop/ui/src/modules/desktop-base/seam-parity.test.tsx` — asserts: for each converted seam (`hardware.monitor-button`, `layout.sidebar-header-spacer`) — with the desktop registration applied the seam resolves to the desktop variant, and without it renders the core fallback; locks the behavior of the deleted shadows into the new mechanism.
...
- **TEST-12** (tier: unit) [covers: ITEM-13, ITEM-14] file: `src-app/ui/scripts/gen-override-registry.test.mjs` — asserts: `parseShadowExceptions` captures approved shadow paths (incl. hyphens/slashes like `api-client/types.ts`) and rejects an exception with no `[approved:]` token; the raw-shadow gate flags an unaccounted shadow (a new desktop-tree file with a ui sibling) and passes when every shadow is a seam/`.desktop`/exclusive/approved-exception. The full migration (ITEM-13) is additionally proven green by TEST-8 (both seams' parity), TEST-4 (resolver tiers), the desktop `vite build`, and `npm run check` (which runs the gate) in both workspaces.
...
- ITEM-11 → TEST-11
- ITEM-12 → TEST-7
- ITEM-13 → TEST-12 (+ TEST-8 both-seam parity, TEST-4 resolver, desktop build, npm run check)
- ITEM-14 → TEST-12
