...
- **TEST-6** (tier: unit) [covers: ITEM-9] file: `src-app/ui/src/modules/layouts/app-layout/components/Drawer.test.tsx` — asserts: after the seam conversion, the Drawer's swipe-to-close handler and the `higherLayerOpen` stacking guard are present on BOTH web (fallback) and desktop (override) render paths — i.e. the previously-dropped behavior is restored, not re-dropped.
...
- **TEST-8** (tier: unit) [covers: ITEM-8] file: `src-app/desktop/ui/src/modules/desktop-base/seam-parity.test.tsx` — asserts: parametrized over the 3 converted seams (Drawer, SettingsPage, HardwareMonitorButton) — with the desktop registration applied each seam resolves to the desktop variant, and without it each renders the core fallback; locks the behavior of the deleted shadows into the new mechanism.
