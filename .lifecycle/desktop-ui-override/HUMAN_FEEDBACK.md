...
- **FB-8** [status: resolved] — merge-gate found a merge-BLOCKING bug: the gate read approved SHADOW-EXCEPTIONs from `.lifecycle/DECISIONS.md`, which is STRIPPED at merge, so on main it found 0 exceptions → 3 UNACCOUNTED shadows → `npm run check` fails forever for everyone → verified in a clean staging merge. → Moved the approval source of truth to a PERMANENT committed file `src-app/desktop/ui/OVERRIDE_EXCEPTIONS.md`; the gate reads that (dropped the `.lifecycle` read). Verified the gate + manifest regen pass with `.lifecycle` REMOVED, in both workspaces. TEST-12 asserts the permanent file exists + carries the 3 approvals so it can't regress. [generalizable: yes — any gate that runs in `npm run check`/CI must read its config from a PERMANENT product-tree path, NEVER from `.lifecycle/` (stripped at merge) — verify every such gate against a lifecycle-stripped tree]

- **FB-7 (merge)** [status: resolved] — "merge current main + find NEW shadows" → merged origin/main (68 behind → 0); one conflict (a generated report) resolved; testid plugin auto-merged keeping both my fix + main's allowlist; openapi/types.ts 0-line diff vs main (no backend changes); **0 new shadows** (main added no desktop overrides — verified by the gate AND a raw enumeration); both `npm run check` + e2e + 9/9 green on the merged base. [generalizable: no]
