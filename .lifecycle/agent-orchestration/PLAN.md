...
- **ITEM-61**: **Per-surface, model-window-aware trigger (LOCK-6).** Budget on the SPECIFIC model's window + reserved output headroom (`window − maxOutput − safety`), **admin-tunable per surface**: **chat = eager** (Tier-4 ≈60% window, oldest-30%/keep-70% split); **agent = patient** (Tier-4 ~75%, match Claude Code). Tier 0 per-result (always); Tier 1 ~50%. Use a **tokenizer-accurate** count (not `chars/4`) so real overflow can't precede the estimate. Add a manual **`/compact`-with-focus** affordance. (Replaces the current one-size eager soft-limit.)
...
- **LOCK-6 — Compaction/summarization (Group I) defaults, human-selected 2026-07-19 — PER-SURFACE (admin-tunable per surface; precedent: chat host 200K vs workflow host 100K soft limits already differ):**
  - **Chat (interactive) = eager:** when it fires, summarize the **oldest ~30% / keep newest ~70%** (the existing `keep_fraction=0.7` split), on an **earlier trigger** (≈60% window) — keeps interactive chat clean/responsive.
  - **Agent (autonomous / workflow / background) = patient:** raise the Tier-4 trigger to **~75% of the model window** (match Claude Code / Anthropic API compaction default) so long autonomous runs keep more working context before paying for a compaction; same ~30/70 split when it fires.
