...
- **ITEM-61**: **Fix the trigger — last-resort, model-window-aware, not eager.** Replace the eager ~30%-at-soft-limit with a budget on the SPECIFIC model's window + reserved output headroom (`window − maxOutput − safety`): Tier 0 per-result (always); Tier 1 ~50%; Tier 4 ~80–85%. Use a **tokenizer-accurate** count for the trigger (not `chars/4`) so real overflow can't precede the estimate. Add a manual **`/compact`-with-focus** affordance.
- **ITEM-62** [stretch]: **Async "sleep-time" consolidation** — a background pass that consolidates/rewrites memory blocks + the running summary between turns (Letta sleep-time compute), improving the live context without paying inline. Rides the Group-D background backbone.
