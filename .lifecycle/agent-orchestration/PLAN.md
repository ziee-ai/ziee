...
- **ITEM-28**: [DESCOPED] Live agent TODO checklist (UI) — the render is **absorbed into ITEM-36** (Group G's live checklist render); no separate work item. See DECISIONS.md.
...
- **ITEM-30**: [DESCOPED] Sandbox background output backpressure (ring / head+tail buffer, byte-range paging) — the execute path is in the `sdk` submodule (`ziee-sandbox`), a separate repo with its own lifecycle. The in-tree background sandbox exec (T25) is complete + functional (runs detached, collects the full result envelope under the existing 600s wall-clock + resource caps). The ring-buffer/paging refinement is a `ziee-sandbox` change → deferred to a dedicated sdk PR. See DECISIONS.md DESCOPED: ITEM-30.
- **ITEM-31**: [DESCOPED] Sandbox background lifetime policy (idle/no-new-output reaper, admin absolute-max/idle columns, `cgroup.kill` grandchild reap, mid-command streaming detach) — same rationale as ITEM-30: this is `ziee-sandbox` (sdk submodule) systems work (changing the crate from run-to-completion to streaming-detach + cgroup manipulation). In-tree (T25): `timed_out` IS reported distinctly, `kill_on_drop` reaps the child on cancel, every existing hardening guard is preserved, and the 600s wall-clock + `code_sandbox_settings` caps bound the run. The idle-reaper + cgroup.kill + streaming refinements → deferred to the dedicated sdk PR. See DECISIONS.md DESCOPED: ITEM-31.
...
- **ITEM-33**: [DESCOPED] Named reusable agent definitions + cumulative per-conversation spawn budget + streaming child progress + per-child sandbox/approval mode — the Group-A ergonomics/governance polish; **not selected by the human**, deferred to a post-A round. See DECISIONS.md.
