...
## 11. Context management / compaction (Group I) — dedicated pass + as-built reconciliation

Human: "our summarization isn't on par with SOTA — refactor it." A SOTA pass + an as-built map.

**As-built (ziee):** TWO summarizers. (1) agent-core `Compactor`/`CompactionExtension` — single-stage, **eager**, position-based: on a soft limit, summarize oldest ~30% into **one freeform System block**, keep pinned System + newest 70%. (2) the chat `summarization` module — the *richer* one: full + **incremental running-summary**, admin-tunable prompts, token-aware boundary cutoff, persisted `conversation_summaries(branch_id)`. **In the agent-core chat path BOTH run and write the same row with incompatible semantics → last-writer-wins corruption + double LLM cost.** Tool-output trim (`clear_old_tool_results` + `tool_result_mcp` recall) exists **legacy-loop-only**, not in the Compactor/engine. Keep is purely position-based (no task/error-signal retention). `.order()` is inert (Vec-insertion order). Estimator is chars/4.

**SOTA (primary):** the field converges on a **tiered ladder, cheapest→most-lossy, summarize LAST**:
- **Anthropic ships 3 composable primitives:** **context-editing** `clear_tool_uses_20250919` (clear stale tool results, default `keep=3`, trigger 100K tokens, `clear_at_least` cache-thrash guard, `exclude_tools`) — the cheap non-LLM tier; **compaction** `compact_20260112` (whole-conversation summary, default trigger **150K input tokens**, min 50K, `instructions` replaces the prompt, `pause_after_compaction`) — the last resort; **memory tool** `memory_20250818` (client-side `/memories` files, auto-injects "ASSUME INTERRUPTION — record progress", pairs with context-editing to save-before-clear). Eval: context-editing **+29%**, +memory **+39%**, **84% token reduction** over a 100-turn eval.
- **Anthropic "effective context engineering":** "safe starting point = clear cached tool results"; summarization is heavier; **preserve architectural decisions, unresolved bugs, implementation specifics**; sub-agents return **1–2K-token** summaries.
- **Claude Code 5-level pipeline** [secondary, multi-source-consistent]: (1) Tool-Result Budget (>50K chars → disk + ~2K preview + re-read) → (2) History Snip → (3) Microcompact (drop old tool results, cache-aware) → (4) Context Collapse (~90%, a view overlay) → (5) Autocompact (~89–92%, a child agent writes a **9-section** structured summary). CLAUDE.md + todo list survive by re-injection.
- **Cursor:** RL-trained **concise ~1K-token** structured summaries (plan state, remaining tasks, errors/fixes, file paths); reward applied to the summary tokens; "compaction can make the model forget critical info."
- **Letta:** core (RAM, pinned editable blocks) ↔ recall ↔ archival (disk); evict ~70% + **recursive** summary (summarize-with-existing-summary); **sleep-time** async memory consolidation.
- **LangGraph/LangMem:** `SummarizationNode` + `RunningSummary` (never re-summarize the same msgs) + `trim_messages` (cheap keep-last). Decide: trim vs summarize vs offload.
- **Stable signals (union template):** verbatim latest user request/intent, current task/todo + plan state, key decisions + rationale, files/edit state, open errors + fixes, WIP + next step, recall handles, pinned/core blocks. **Structured beats freeform.** Triggers are **%-of-window with reserved output headroom, last-resort — eager summarization is treated as a quality bug.** Recall = handle + re-fetch (tool_result_mcp / memory files / archival search). Failure modes: summarize-too-early, lose the task/plan, drop a needed error, **governance/constraint decay through repeated compaction**.

**Refactor → Group I (ITEM-56–62):** unify the two summarizers into one agent-core pipeline + fix the double-write (56); Tier 0/1 tool-result budget+eviction before summary (57); Tier 2 turn trim (58); Tier 3 save-before-clear offload to `assistant_core_memory`+`memory` (59); Tier 4 structured sectioned summary last-resort, pin the Group-G task list, governance-decay guard (60); model-window-aware last-resort trigger + tokenizer-accurate count + manual `/compact` (61); stretch sleep-time consolidation (62). ziee already owns Tiers 0/1 + the external-memory tier — the fix is ordering + unification + structure + a later trigger.

Sources: platform.claude.com/docs/en/build-with-claude/{compaction,context-editing} + /agents-and-tools/tool-use/memory-tool + claude.com/blog/context-management + anthropic.com/engineering/effective-context-engineering-for-ai-agents · developers.openai.com/api/docs/guides/compaction · cursor.com/blog/self-summarization · letta.com/blog/{agent-memory,sleep-time-compute} · langchain-ai.github.io/langmem. Flagged secondary: Claude Code / Codex CLI internal stage names+thresholds (community reverse-engineering, cross-source-consistent, not official — API primitives above are the authoritative version).
