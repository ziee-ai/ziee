...
- **ITEM-33**: Frontend surfacing — paused-state badge + `paused_reason`, a
  ...
  list. Mirrors settings-card + `McpServerDrawer` "Calls" tab. (The
  continue-in-chat button is dropped with ITEM-32.)
...
- **ITEM-36**: Change-detection engine (backend, `change.rs`) — after a successful firing, reduce the result to a normalized `fingerprint` + an `item-set` (extract identifiable IDs — DOI/PMID/arXiv/URL — via `lit_search::dedup`/`citations` normalization; fall back to a normalized-text hash). Compare to the task's stored `last_result_fingerprint`/`last_result_signature_json`; when `notify_on='on_change'` and unchanged, record the run `completed` (no-change) and **suppress** the notification; when changed, compute the **delta** (set-diff → "N new") and pass it into the notification body. Then persist the new fingerprint/signature. `always` mode notifies every success.
- **ITEM-37**: Change-detection surfacing (frontend) — a `notify_on` control in the form drawer ("Notify every run" vs "Only when something changed") and delta rendering in the notification/inbox ("2 new since last run"). Mirrors the `notify_mode` toggle placement.
...
- **Change-detection / delta (ITEM-36)** → `lit_search::dedup` (`normalize_doi` + ID normalization) and `citations` fingerprinting for the exact item-set diff; the "reduce a result to a stored signature + diff next time" shape mirrors how `citations` computes `dedup_fingerprint`.
