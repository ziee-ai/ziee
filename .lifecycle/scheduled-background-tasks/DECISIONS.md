...
### DEC-17: Recurring task → conversation threading (the follow-up question). [CONFIRMED — user-chosen]
**Resolution:** (User confirmed "one bound conversation".) For the `prompt` kind, a recurring task owns ONE **bound conversation**; each firing appends a turn to it (rather than a fresh conversation per run), so following up on a result is native — open the task's conversation and keep chatting, with full history in one place. Deleting that conversation **pauses** the task. Unbounded context growth is bounded by the existing summarization / `clear_old_tool_results` machinery. Each firing is still recorded in `scheduled_task_runs` for a clean audit list.
...
### DEC-20: "Only notify if changed" / meaningful-change detection. [IN V1 — user-chosen]
**Resolution:** IN V1 (user chose to pull it in). A per-task `notify_on: always | on_change` mode (ITEM-36/37). On a successful firing the result is reduced to (a) a normalized **fingerprint** and (b) where the result carries identifiable items (DOIs/PMIDs/arXiv IDs/URLs), an **item-set**, both persisted on the task (`last_result_fingerprint`, `last_result_signature_json`). Next run: if `notify_on='on_change'` and the fingerprint/item-set is unchanged, the firing is recorded as `completed` (no-change) but **no notification is sent**; if changed, the notification body leads with the **delta** ("2 new papers since last run"), computed by set-diff for identifiable items and falling back to a "result changed" flag otherwise. `always` mode notifies every successful run.
**Basis:** user + landscape — the user pulled this into v1 for the life-science literature use case; it's the top monitoring differentiator (Firecrawl /monitor "nothing changed → nothing sent," ~90% fewer tokens; Google Scholar-alert users want only-new/dedup). Item-set diffing reuses the in-tree `lit_search::dedup`/`citations` DOI-normalization so the delta is exact for scholarly results, not an LLM guess.
