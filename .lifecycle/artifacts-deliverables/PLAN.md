...
- **ITEM-21** (deferred → v1.1, see DRIFT-3): Image upload/paste embed — in the markdown WYSIWYG, drag-drop/paste uploads via `POST /api/files/upload` (existing size/type limits) and inserts a markdown image reference (survives serialize as a link). Not implemented in v1.
...
- **ITEM-15** (deferred → v1.1, see DRIFT-3): Selection → query (non-mutating) — a selection popover in the canvas with "Ask about this": the selection is quoted into the chat composer as context referencing the file; the model answers in chat; no mutation, no new backend. Real-LLM-result dependency.
- **ITEM-16** (deferred → v1.1, see DRIFT-3): Selection → edit (mutating) — "Edit this section" in the popover sends the selection + instruction so the model runs a targeted `edit_file(old_str=<selection>)` landing as a new version; degrades to instruction-only if the selection is not a unique substring; reuses `files_mcp::edit_file`. Shaping logic built + unit-tested; real-LLM-result dependency.
