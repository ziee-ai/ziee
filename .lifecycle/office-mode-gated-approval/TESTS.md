...
- **TEST-13** (tier: integration) [covers: ITEM-5, ITEM-7] file: `src-app/server/tests/mcp/mcp_approval_workflow_test.rs` — asserts: the EXISTING approval-workflow suite (auto-approve executes immediately / manual-approve creates a pending approval / approve-and-resume executes / auto-approved-tools skip the prompt) STILL passes end-to-end after the `compute_needs_approval` extraction — i.e. the extraction is behaviour-preserving for control / builtin / manual / auto-approved servers through the SAME loop that now also gates `office_bridge`.
