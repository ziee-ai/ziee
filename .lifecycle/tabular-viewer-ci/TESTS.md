...
- **TEST-27** (tier: e2e) [covers: ITEM-5] file: `src-app/ui/tests/e2e/visual/tabular-viewer.spec.ts` — asserts: Copy-selection with nothing selected warns and leaves the clipboard empty (no whole-view fallback); and the Export button's aria-label names the format (`Export view (CSV)`).
