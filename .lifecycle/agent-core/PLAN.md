...
- **ITEM-7**: [DESCOPED] Subagent fan-out crate module (`fanout.rs`) is unit-tested but NOT wired into the loop this pass (`fan_out` has no production caller; `allow_delegate` never true). **Subagent fan-out (C2)** — (C2)** — `AgentCore::fan_out(children, cancel) -> Vec<SubagentSummary>`: fresh child per spec (own budget), bounded `SubagentLimits {max_depth=1, max_threads=6}` (semaphore), summaries-not-transcripts. Per-child `model_id` resolved via the injected **`ModelResolver`** (RBAC-bound); child `ToolProvider::list` omits `delegate` (enforces `max_depth=1`).
...
- **ITEM-10**: [DESCOPED] tool `concise|detailed` verbosity convention not implemented this pass (no `verbosity` symbol in the crate). **Tool-ACI `concise|detailed` convention (C5)** on `concise|detailed` convention (C5)** on built-in tool results (concise default), threaded through `structured_content`.
...
<!-- === F. Host 3 — fan-out === -->
- **ITEM-27**: [DESCOPED] the `delegate` core-injected tool is not wired (depends on the descoped ITEM-7 fan-out). `delegate` **core-injected** tool exposing `fan_out` in chat AND workflow — "research each of these N in parallel and summarize." Children inherit the user's accessible tools but NOT `delegate` (enforces `max_depth=1`), RBAC-scoped, summaries only. (Core-injected → no A8.)
...
- **ITEM-29**: [DESCOPED] no new agent-authoring/run-view UI built this pass (the chat cutover reused the existing chat UI; the workflow agent step runs headless + via the existing run view). Workflow **agent-step authoring UI** + **run-view agent surface** (live tool-call stream, plan/todo, the durable review-gate form pre-filled via elicit `data:`).
- **ITEM-30**: **Agent admin settings page** `/settings/agent` (mirror the closest app settings card); gated `agent::settings::read`.
- **ITEM-31**: [DESCOPED] no plan/todo + progress renderer built this pass (no agent chat/run UI surface). **Plan/todo + visible-progress rendering (C3/F3)** in + visible-progress rendering (C3/F3)** in chat + run views (a content-type renderer + live activity stream).
