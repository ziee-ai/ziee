...
- **ITEM-5** — verdict: PASS — `clamp_wav_tail` is a pure PCM/RIFF tail-slice (best-effort no-op on unparseable), unit-testable like the existing `wav_duration_secs`; sharing `forward_to_whisper` is a visibility change only.
...
- **ITEM-10** — verdict: PASS — three fields in the existing react-hook-form/zod card; bounds mirror the backend; `VoiceConfig.store` already generic.
