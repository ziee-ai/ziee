...
- **TEST-15**: ENV-BLOCKED (desktop host) — `voice-desktop-surface.spec.ts`. Diagnosed: the desktop app boots fine (Vite + React render) but stays on the **login screen** ("Welcome back") because the **tauri auto-login mock is non-functional in this box** — so EVERY desktop spec needing a session fails the same way: MY spec, the pre-existing **TEST-30** in the same file, AND unrelated `desktop-auto-login.spec.ts` (4/4 fail waiting for `desktop-bootstrap-failed`/menu). This is a shared desktop tauri-mock/harness issue (B3: not mine to edit), NOT this diff — my only desktop changes are the generated `types.ts` + this spec; no desktop app code. Per the cross-platform testing setup, desktop e2e runs on the **dedicated desktop host** (real tauri runtime). Desktop parity of the streaming voice code is covered by (a) `npm run check (desktop/ui): PASS` (tsc + lints on the desktop bundle) and (b) the 22 passing ui-core `14-voice` specs, which exercise the SAME glob-shared voice code the desktop bundle ships.

  `gate:ui (desktop/ui)`: same block — the desktop workspace is flagged "touched" only by this test spec + the mechanically-generated `types.ts` (no desktop product code), and its runtime canary needs the same non-functional desktop harness. Runs on the dedicated desktop host.
