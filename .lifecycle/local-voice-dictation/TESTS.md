...
- **TEST-13** (tier: integration) [covers: ITEM-12, ITEM-13, ITEM-15] file: `src-app/server/tests/voice/permissions_test.rs` — asserts: transcribe → 401 without a token, 403 for a user lacking `voice::transcribe`, allowed for a default `Users` member (migration 133); admin endpoints 403 for a non-admin.
