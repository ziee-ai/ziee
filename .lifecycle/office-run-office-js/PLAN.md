...
- **ITEM-2**: Remove the `edit_document` tool descriptor (and its `op` enum) from `tool_list()`; update the `tool_list` unit test to assert the new exact 7-tool set (drops `edit_document`, adds `run_office_js`).
