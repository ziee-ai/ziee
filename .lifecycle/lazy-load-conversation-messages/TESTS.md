...
- **TEST-14** (tier: unit) [covers: ITEM-12] file: `src-app/server/src/modules/chat/core/types/message.rs` — asserts: `MessageSearchQuery` rejects a blank/whitespace `q` (→ empty result, not a full scan) and clamps its result cap; the snippet builder trims/ellipsizes around the match to a bounded length.
...
- **TEST-15** (tier: integration) [covers: ITEM-12] file: `src-app/server/tests/chat/messages_test.rs` — asserts: the in-conversation search endpoint returns matches by MESSAGE TEXT across the whole active branch (including messages that a tail load would NOT return), ordered branch-chronologically with correct 1-based `ordinal` + `total`, a snippet around the hit, `truncated` when over the cap, empty result for a blank `q`, active-branch scoping (a match only present in a superseded edit branch is NOT returned), and owner-scope (cross-user conversation → 404).
