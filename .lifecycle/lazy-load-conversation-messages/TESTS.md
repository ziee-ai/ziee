...
- **TEST-13** (tier: unit) [covers: ITEM-5] file: `src-app/server/src/openapi/emit_ts.rs` — asserts: the golden parity test `openapi::emit_ts::tests::types_ts_parity` regenerates `types.ts` from the committed `openapi.json` and byte-matches the committed `types.ts` — i.e. it FAILS if `MessageHistoryQuery`/`PaginatedMessages`/`MessageSearch*` were added without running `just openapi-regen`, backstopping ITEM-5 in both binaries.
- **TEST-14** (tier: unit) [covers: ITEM-12] file: `src-app/server/src/modules/chat/core/types/message.rs` — asserts: `MessageSearchQuery` treats a blank/whitespace `q` as empty (→ empty result, no scan), clamps `per_page` to 1..=100 (default 25) and floors `page` at 1; the snippet builder trims/ellipsizes around the match to a bounded length.
...
- **TEST-15** (tier: integration) [covers: ITEM-12] file: `src-app/server/tests/chat/messages_test.rs` — asserts: the in-conversation search endpoint returns matches by MESSAGE TEXT across the whole active branch (including messages a tail load would NOT return), ordered branch-chronologically; PAGINATION works — `page`/`per_page` return the right slice, `total` is the full match count across pages, and `ordinal` is globally continuous across page boundaries (page 2's first ordinal = per_page+1); each match carries a snippet around the hit; blank `q` → empty; active-branch scoping (a match only in a superseded edit branch is NOT returned); owner-scope (cross-user conversation → 404).
...
- **TEST-16** (tier: e2e) [covers: ITEM-12, ITEM-13] file: `src-app/ui/tests/e2e/chat/conversation-find.spec.ts` — asserts: (this updates the existing spec to the server-backed flow) opening find and searching a term that ONLY appears in an unloaded (older) message surfaces it in the results list with a snippet and the "X of Y" count from the server total; selecting/Next-ing to that result JUMPS to the message (around= load), centers + highlights it; from there scrolling up/down "loads more around" the found message (before/after pagination continues). A term with no matches shows "No results".
