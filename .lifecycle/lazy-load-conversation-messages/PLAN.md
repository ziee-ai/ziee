...
- **ITEM-12**: Backend in-conversation message SEARCH endpoint: `GET /conversations/{id}/messages/search?q=<term>&page=<p>&per_page=<n>` (handler + repo fn + `_docs`). Server-side, case-insensitive substring (`ILIKE`) over `message_contents.content->>'text'` scoped to the conversation's ACTIVE branch (mirrors the active-branch EXISTS-join in `conversations.rs::list_conversations` + its paginated `count`), returning a PAGE of matches ordered branch-chronologically: `MessageSearchResults { matches: MessageSearchMatch[], total, page, per_page }` where `MessageSearchMatch { message_id, role, created_at, snippet, ordinal }` and `ordinal` is the 1-based GLOBAL position within the full match set (page*per_page + i) for a stable "X of Y". `per_page` default 25, clamp 1..=100; blank `q` → empty. This is what makes find work over UNLOADED messages under lazy-load.
- **ITEM-13**: Upgrade the F3 find UI (`ConversationFindBar.tsx` + `Chat.store.ts`) from client-side `findMatches` to the ITEM-12 paginated server-side search: debounced query → `Message.searchInConversation` (page 1); DISPLAY the results as a selectable snippet LIST under the bar that infinite-scrolls to fetch the next page as the user scrolls it; keep the "X of Y" readout (Y = server `total`) with Next/Prev that step across the FULL match set (fetching the adjacent page when stepping past the loaded boundary). Selecting/navigating a match scrolls it into view when it's in the loaded message window, else calls `jumpToMessage(matchId)` (ITEM-7, `around=`) then scroll-centers + highlights (reuse `ConversationFindContext`); after a jump, the before/after infinite-scroll (ITEM-9) lets the user "load more around" the found message. `findMatches.ts` is reduced to the pure snippet/highlight helper — the MATCH SET now comes from the server so unloaded matches are found.
...
- **In-conversation search endpoint** (ITEM-12): mirror the ACTIVE-BRANCH text-match EXISTS-join in `repository/conversations.rs::list_conversations` (`message_contents mc JOIN branch_messages bm2 ON ... WHERE bm2.branch_id = <active> AND mc.content_type='text' AND mc.content->>'text' ILIKE '%'||$q||'%'`) with a matching `count` query for `total` (mirror `conversations.rs::count_conversations`), LIMIT/OFFSET pagination on the ordered match set, and the handler/`_docs`/`Query`-type shape of `mcp/tool_calls` (page/per_page) + the sibling `get_conversation_history`.
