...
### DEC-7: Do the skill drawer and workflow step output get the FULL `preprocessMarkdown`, or a math-only entry point?
**Resolution:** Full `preprocessMarkdown`. These two surfaces also gain reference-link inlining and blocked-image placeholders.
**Basis:** user — presented as an explicit option picker and the human chose "Route all sites through preprocessMarkdown", then on review asked that the broadening be deliberate rather than incidental. Two binding obligations follow: (a) verify BOTH added behaviors actually render correctly on those two surfaces (by running, per rule B7 — TEST-10, TEST-11), and (b) disclose the broadening in a dedicated "Beyond #177" section of the PR body. Documented fallback: if (a) surfaces a problem, scope those two files to `normalizeMathDelimiters` only and record the reason as a drift entry. Rationale for the chosen option: a second entry point is exactly the drift the shared pipeline exists to prevent, and both added behaviors already ship on the file-viewer path.
