...
- **TEST-6** (tier: integration) [covers: ITEM-3] file: `src-app/server/tests/chat/title_approval_test.rs` — asserts: the `TitleUpdated` SSE event is observed on the stream BEFORE the terminal chunk on the skipped path — the `start_generation` drain invariant (`streaming.rs:953-964`). An event emitted after the terminal chunk would be silently dropped, so DB-only assertion is insufficient.
