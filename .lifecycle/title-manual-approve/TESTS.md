...
- **TEST-13** (tier: e2e) [covers: ITEM-8] file: `src-app/ui/tests/e2e/chat/untitled-conversation-label.spec.ts` — asserts: in the **PaneManagerDrawer** (`pane-manager-search` / `pane-manager-list`), typing first-message text FINDS the conversation, and typing "Untitled" no longer matches one that renders a real preview. **Corrected after an A/B run:** the first version asserted against `RecentConversationsWidget`, whose search resolves SERVER-SIDE and which ITEM-8 does not touch — it failed identically with and without the change, i.e. it was blind to the feature. The drawer and the picker pane are the only two surfaces that filter the already-loaded list client-side.
