...
- **TEST-21** (tier: e2e) [covers: ITEM-16, ITEM-18, ITEM-22] file: `src-app/ui/tests/e2e/14-knowledge-base/kb-chat-retrieval.spec.ts` — asserts: attach a KB to a chat, send a question, the agent's turn shows a `search_knowledge` call, a numbered citation chip, and the expandable retrieval-transparency panel listing retrieved chunks; clicking the chip opens the file viewer at the cited page. (Real-LLM-gated for the answer; the attach + chip + transparency panel + deep-link are asserted on a seeded tool-result even when the LLM is skipped.)
...
- **TEST-26** (tier: unit) [covers: ITEM-23] file: `src-app/server/src/modules/knowledge_base/tools.rs` — asserts: the `search_knowledge` tool description contains the grounding instruction (answer only from returned results / say "not found") and the chat-extension note carries it; guards the copy that drives grounded-answer behavior.
- **TEST-27** (tier: unit) [covers: ITEM-22] file: `src-app/ui/src/modules/knowledge-base/components/KnowledgeRetrievalTransparency.tsx` — asserts: the transparency mapper turns a `search_knowledge` `structuredContent` result into the per-chunk rows (file name, page, score, snippet) the panel renders, and renders an empty state when no hits.
