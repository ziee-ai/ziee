...
- **TEST-11** (tier: unit) [covers: ITEM-3, ITEM-4] file: `src-app/server/src/modules/mcp/resource_link.rs` — asserts: `trusted_hosts_from_servers` EXCLUDES `is_system` (built-in) servers so their loopback `url` (`127.0.0.1`) never enters the trust set — a regression guard for the loopback-SSRF hole where an external `resource_link` at `http://127.0.0.1:<port>` would otherwise gain same-host trust; only user-owned non-system server hosts are trusted.
