...
- **FB-3** [status: resolved] — "With the prompt you provided, the message disappear for a very slice of time, showing the warning message no tool ... then the response appear again" + "Work on this PR please ... check to see if you need to pull remote khoi" → The single-turn fix did NOT cover the multi-tool/tool-approval flow. Merged current `origin/khoi` (bringing `#137` tool-pairing + `#138` stale-artifact, which resolve the multi-tool empty-completion NOTICE on the backend), then fixed the remaining frontend RESUME disappear: the streaming placeholder overwrote the existing assistant row with empty content on a resume → bubble vanished. Reuse the existing row instead (`resumeOrFreshPlaceholder`). Verified live (real gpt-oss + approvals, merged backend). [generalizable: yes — a "fix" for a streaming-flicker symptom must be reproduced in the ACTUAL user flow (multi-tool + tool-approval, not just a single turn) with the real model before claiming done]
