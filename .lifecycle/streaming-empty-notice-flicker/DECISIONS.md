...
### DEC-7: How to test the resume-chain placeholder fix — unit helper + live, or a full approval-flow e2e?
**Resolution:** Extract the decision into the pure `resumeOrFreshPlaceholder` helper and unit-test it
(TEST-6); validate the end-to-end resume behavior LIVE (real gpt-oss + tool approval against a
merged-code backend, confirmed over multiple runs). Do NOT add a bespoke multi-send-same-message-id
SSE-mock e2e.
**Basis:** convention + codebase — the disappear is a transient (placeholder overwrite within one
handler tick); a faithful SSE-mock of the approval→resume-with-same-assistant-id flow is high-cost
and timing-flaky, whereas the pure helper locks the exact fix decision and the live runs prove the
integration (mirrors DEC-5's cost/signal reasoning for the cancel path).
