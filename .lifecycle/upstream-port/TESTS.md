...
- **TEST-8** (tier: unit) [covers: ITEM-3] file: `src-app/server/src/modules/web_search/mod.rs`, `src-app/server/src/modules/lit_search/mod.rs` — asserts: the existing in-module unit tests for both modules still pass after the struct + router split.
  **NOT VERIFIED — stated rather than claimed:** no test on this branch drives an
  actual `POST /api/run-js/mcp` against a server booted with `js_tool.enabled: false`
  and asserts 404. That is the assertion that would prove INV-2 end-to-end, and it
  needs a `TestServerOptions` knob upstream does not expose. The guard is proven by
  construction (`register_routes` returns the router unmerged) and by TEST-5's
  control, not by an HTTP round-trip. Flagged in the PR body.
...
- **TEST-11** (tier: unit) [acceptance] [invariant: INV-6] [covers: ITEM-6] file: `src-app/server/src/modules/llm_model/handlers/downloads.rs` — asserts: `wire_shape_tests` — the SSE payload stays FLAT (a nested `progress_data` would silently re-break every consumer), and an absent progress row serialises the figures as `null` rather than `0`. This is the server-side half of the contract TEST-9/10 consume.
...
- **TEST-15** (tier: unit) [covers: ITEM-7, ITEM-8] file: `src-app/server/build.rs`, `src-app/server/build_helper/pgvector.rs`, `src-app/desktop/tauri/tauri.conf.json` — **PARTIALLY VERIFIED.** The `cargo:warning=` conversion is exercised on every build of this branch (the helper runs). The `MACOSX_DEPLOYMENT_TARGET` line sits inside `target.contains("apple")` and is **unreachable on Linux**, so it is NOT verified here — no Darwin toolchain, and the brief forbids macOS builds. Likewise `beforeBuildCommand` is only executed by a real `tauri build`. Both are recorded as not-verified-on-this-platform rather than claimed.
