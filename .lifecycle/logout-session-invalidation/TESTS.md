...
- **TEST-18** rescoped [covers: ITEM-11] — now asserts `setAuthFromAutoLogin` **PRESERVES** permissions on a same-identity re-mint, plus **TEST-18b** pinning why that is safe (the only identity-changing caller passes `user: null` and stays unauthenticated until `/me`). The previous version asserted the opposite and encoded a real regression for desktop/tunnel.
