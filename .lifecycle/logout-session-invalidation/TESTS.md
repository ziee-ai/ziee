...
## Added in FIX_ROUND-2 (fresh blind round)

- **TEST-25** (tier: integration) [covers: ITEM-14] file: `src-app/server/tests/auth/session_refresh_test.rs` — asserts: **DETERMINISTIC** proof that the claim path takes the `users` row lock. Holds exactly the lock a logout holds (`UPDATE users` → FOR NO KEY UPDATE, which conflicts with the claim's FOR SHARE), fires a real `POST /auth/refresh`, asserts it does NOT complete for 1.5s, then completes with 200 once the lock is released. **Negative-controlled: with the `FOR SHARE` removed it FAILS** ("refresh completed while the users row lock was held"). TEST-23 (the HTTP race) is probabilistic and cannot guarantee it hits the window; this is the proof.
- **TEST-18** (tier: unit) [covers: ITEM-11] file: `src-app/ui/src/modules/auth/Auth.store.test.ts` — asserts: **RESCOPED in FIX_ROUND-2** — `setAuthFromAutoLogin` PRESERVES `permissions`/`hasPassword` on a same-identity re-mint (desktop `applyTokens` / tunnel `applySession` never call `initAuth()` afterwards, so clearing would strand them at `[]` for the whole session). Its previous version asserted the OPPOSITE and encoded a real regression.
- **TEST-18b** (tier: unit) [covers: ITEM-11] file: `src-app/ui/src/modules/auth/Auth.store.test.ts` — asserts: an identity-CHANGING callback (`user: null`, the OAuth path) stays `isAuthenticated: false` until `/me` resolves — which is *why* TEST-18 is safe: no authenticated render can ever observe the previous identity's grants.
