...
- **ITEM-12**: The three capability fields round-trip through the **existing** `capabilities` JSONB create/update path with **no handler change** (adding them to the DB `ModelCapabilities` struct is sufficient). Silent rows (fields unset) resolve dynamically from catalog + family policy at request time — no create-time auto-baking (which would freeze inference). The editable row is the source of truth once a user sets a field.
