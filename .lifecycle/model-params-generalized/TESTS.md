...
- **TEST-19** (tier: integration) [covers: ITEM-11] file: `src-app/server/tests/chat/stub_chat_tier2_test.rs` — asserts: `normal_text_completion_reports_stop` — the provider's finish_reason flows through the full chat path and the Complete frame carries the canonical `"stop"`; `empty_completion_reports_finish_reason_empty` confirms the `"empty"` override still applies after canonicalization. (Deterministic, stub-backed — the real-LLM `mcp_loop_settings_test.rs` requires provider keys the deterministic-mocks constraint precludes.)
