...
- **TEST-4** (tier: unit) [covers: ITEM-5] file: `src-app/ui/src/modules/file/viewers/tabular/parse-cap.test.ts` — asserts: `parseDelimitedText` on a CSV with more than 10k data rows returns ALL rows (no 10k head-truncation) with `truncated:false`, and reports `truncated:true` (slicing to the cap) only once `DELIMITED_MAX_ROWS = 300_000` is exceeded — i.e. the retired 10k cap is lifted to the raised OOM backstop.
