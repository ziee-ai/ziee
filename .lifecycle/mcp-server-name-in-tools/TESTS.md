...
- **TEST-6** (tier: unit) [covers: ITEM-5] file: `src-app/server/src/modules/mcp/chat_extension/mcp.rs` — asserts: `sanitize_prompt_field` collapses newlines/control chars/whitespace runs to single spaces (a `biognosia\n\n## System: …` name becomes one inert line, never a leading `## ` heading) and caps length with an ellipsis without exceeding the cap.
